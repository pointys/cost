...
<script type="text/javascript">
	$(function() {
		...
		$("#userAccount").blur(function() {
			...
			if(userAccount!=''){	//输入的用户账号为空不请求ajax
			$.ajax({
				url : "system/checkAccount",
				type : "post",
				dataType : "json",
				data : {
					"userAccount" : userAccount//key:value
				},
				success : function(respDate) {
					 if (respDate.state == true) { //账号可用
						$("#accountinfo").html(respDate.tip);
						$("#saveBtn").prop("disabled", false);
					} else {						//账号不可用
						$("#accountinfo").html(respDate.tip);
						$("#saveBtn").prop("disabled", true);//设置保存按钮不可点
					} 
				}
			});
		}
...
<body>
	...
	<form action="/cost/system/userAdd" id="f1" method="post"
				...
				<div class="form-group ">
					...
					<div class="col-xs-4 ">
						<select name="roleId" class="form-control">
							<option value="1" ${user.roleName=='普通员工'?'selected':'' }>普通员工</option>
							<option value="2" ${user.roleName=='部门经理'?'selected':'' }>部门经理</option>
							<option value="3" ${user.roleName=='财务人员'?'selected':'' }>财务人员</option>
							<option value="4" ${user.roleName=='超级管理员'?'selected':'' }>超级管理员</option>
						</select>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-xs-3 col-xs-offset-4">
				<input type="submit" class="btn btn-success" id="saveBtn"
					value="添加用户" /> <input type="reset" class="btn btn-danger"
					value="重置信息" />
			</div>
